...
function showWindowHref(){
    var firstStr="name=";
    var secondStr="&";
    if(url.value.indexOf(firstStr)>-1){
        var subFirstStr = url.value.substr(url.value.indexOf(firstStr) + firstStr.length, url.value.length);
        var subSecondStr = subFirstStr.substring(0, subFirstStr.indexOf(secondStr));
        url_result.value=subSecondStr;
    }else {
        url_result.value="wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww";
    }
}
...
function timeTest(){
    ...
    function double() {
        ...
        else if((new Date()).getSeconds()<5){clearInterval(interval);}
    }
}
...
function arrSameStr(){
    var mosts=new Array();
    mosts=most.value.split("");
    var mostNumber=0;
    var mostTimes=0;
    for(i=0;i<=9;i++){
        var currentTimes=0;
        for(j=0;j<mosts.length;j++){
            if(mosts[j]==i)
                currentTimes++;
        }
    ...
    result.value="the most number is "+mostNumber+",its times is "+mostTimes+".";
}
